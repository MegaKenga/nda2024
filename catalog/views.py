...
def get_category(request, category_id):
    ...
    products_groups = ProductGroup.objects.filter(categories=category.id)
...
def get_product_with_offers(request, brand_id, product_group_id):
    offers = Offer.objects.filter(product_group=product_group_id)
    product_group = ProductGroup.objects.get(id=product_group_id)
    brand = Brand.objects.get(pk=brand_id)
    context = {'offers': offers, 'product_group': product_group, 'brand_id': brand}
